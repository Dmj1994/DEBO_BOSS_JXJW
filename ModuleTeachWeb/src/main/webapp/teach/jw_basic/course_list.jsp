                ...
                <div class="ibox-content">
                    <form role="form" class="form-inline" id="search_form_small">
                        <div class="form-group">
                            ...
                            <input type="text" name="stuName" placeholder="请输课程名称" class="form-control">
                        </div>
                        <button class="btn btn-primary " type="button" onclick="smallSearch()">&nbsp;搜 索</button>
                        <button class="btn btn-success " type="button" onclick="addCource()"><i
                                class="fa fa-search-plus"></i>&nbsp;&nbsp;新增</button>
                        <button class="btn btn-success " type="button" onclick="addCharpters()"><i
                                class="fa fa-search-plus"></i>&nbsp;&nbsp;新增章节</button>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    ...
    function addCource(){
        windowObj = layer.open({
            title: "新增课程",
            type: 2,
            content: '/teach/jw_basic/cource_add.jsp' ,
            maxmin: true,
            area:['650px','400px']
        });
    }
    function addCharpters(){
        ...
        if (selectedIds&&selectedIds.length==0) {
            layer.alert("请选择需要操作的课程");
            return false;
        }
        windowObj = layer.open({
            title: "新增章节",
            type: 2,
            content: '/teach/jw_basic/charpter_add.jsp' ,
            maxmin: true,
            area:['650px','600px']
        });
    }
